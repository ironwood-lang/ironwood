...
static int mode(const char *name) {
    const char *selected = getenv("IRONWOOD_TEST_PROXY_MODE");
    return active && !strcmp(phase, "negotiation") && selected && !strcmp(selected, name);
}
static void report(void) {
    if (!active) return;
    active = 0;
    fprintf(stderr, "PROXY_COUNT %s recv=%ld send=%ld peek=%ld poll=%ld clock=%ld control=%ld in=%ld out=%ld\n",
            phase, receives, sends, peeks, polls, clocks, controls, received_bytes, sent_bytes);
}
int test_fputc(int value, FILE *stream) {
    if (stream == stdout) {
        ...
        if (value == '\n') {
            line[used] = 0;
            if (!strncmp(line, "@tcp:", 5) && strcmp(line + 5, "end")) {
...
__attribute__((destructor)) static void finish(void) {
    report();
    fprintf(stderr, "PROXY_FDS opened=%d closed=%d\n", opened, closed);
