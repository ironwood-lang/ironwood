...
printf 'RUN - Networking Milestone 5\n'
capture_suite_output networking-m5
IRONWOOD_TEST_NET_M5_SUMMARY=${IRONWOOD_TEST_CAPTURED_OUTPUT##*$'\n'}
if [[ $IRONWOOD_TEST_CAPTURED_STATUS -ne 0 || "$IRONWOOD_TEST_NET_M5_SUMMARY" != "PASS: 4 passed, 0 skipped, 4 total" ]]; then
    printf 'error: networking Milestone 5 suite failed\n%s\n' "$IRONWOOD_TEST_CAPTURED_OUTPUT" >&2
    exit 1
fi
