...
- **Networking status:** N1 pending; Milestones 1 through 4 implemented
...
Beyond completed U5 and networking Milestones 1 through 4, the following work
...
2. review the remaining [networking migration](NETWORKING_MIGRATION_PLAN.md)
   at the [Milestone 4 checkpoint](NETWORKING_MIGRATION_PLAN.md#milestone-4-selection-checkpoint).
   ...
   [M4 evidence](NETWORKING_M4_VERIFICATION.md). Milestones 5 and 6 remain unselected. Event-loop integration remains
...
| [Multithreaded web server](https://doc.rust-lang.org/book/ch21-00-final-project-a-web-server.html) | TCP networking, protocol parsing, concurrency, and worker management | Ironwood's multi-client server target remains single-threaded and event-driven. The implemented representative blocking TCP foundation supports sequential programs; those do not satisfy N1's later event-loop integration gate. |
...
6. **Networking, pending for the near future:** sockets, DNS, HTTP, and
   ...
   sequencing in [D151](DECISIONS.md#d151---stage-blocking-tcp-before-event-loop-integration)
   ...
   non-blocking surface and event-loop integration. This replaces the
   earlier event-loop-before-sockets prerequisite and retains the
...
| [N1: networking foundation](NETWORKING_MIGRATION_PLAN.md) | Pending overall; blocking Milestones 1 through 4 implemented | TCP/DNS and explicit-proxy acceptance programs, scoped TLS and HTTP/HTTPS downloader, and a single-threaded multi-client event-loop server | Java-shaped socket/address APIs, deterministic close plus wrapper reclamation, native error mapping, non-blocking operations and event-loop integration, cross-platform tests |
...
see [M3 evidence](NETWORKING_M3_VERIFICATION.md).
Milestone 4 explicit proxies were selected on 2026-09-16; see
[M4 evidence](NETWORKING_M4_VERIFICATION.md). Milestones 5 and 6 still require separate selection. Even after that migration,
