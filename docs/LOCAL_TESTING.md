...
./scripts/test.sh --test 'rejected free explains both ordinary branch witnesses'
...
./scripts/test.sh --test 'deferred-free registration explains its first failed ownership check'
```
...
free through another alias. The second selection checks the matched bound local
and registration site for pending and duplicate deferred frees, including
