...
| Exceptions and resource cleanup | 💡 Ironwood design | Checked exceptions, `throw`, `catch`, `finally`, causes, and source stack traces are supported. Explicit `close()` and proven-safe `free` replace try-with-resources. |
...
| Packages, imports, and reusable libraries | 💡 Ironwood design | Packages and imports feel familiar. `.ironclass` and `.ironjar` files are compile-time inputs that are rebuilt into one optimized native program. |
| Garbage collection | 💡 Ironwood design | Ironwood has no garbage collector. A compiler-accepted `free` provides deterministic reclamation only when the allocation is proven unobservable afterward. |
