...
| N1 Milestones 1 through 4 TCP/address/host/proxy foundation | Independent Java-compatible facades plus original compiler/runtime mechanisms | `MIT OR Apache-2.0` | Prior OpenJDK source inspection disclosed in [STDLIB_N1_SOURCE_REVIEW.md](STDLIB_N1_SOURCE_REVIEW.md); no upstream implementation, comments or tests copied. Private IP literal parsing and SOCKS negotiation are separately derived and classified below. Public proxy configuration, HTTP CONNECT and descriptor/deadline management remain independently implemented. |
