...
| N1 Milestones 1 through 5 TCP/address/host/proxy/TLS foundation | Independent Java-compatible facades plus original compiler/runtime mechanisms | `MIT OR Apache-2.0` | Prior OpenJDK source inspection disclosed in [STDLIB_N1_SOURCE_REVIEW.md](STDLIB_N1_SOURCE_REVIEW.md); no upstream implementation, comments or tests copied. Private IP literal parsing and SOCKS negotiation are separately derived and classified below. Public proxy configuration, HTTP CONNECT, TLS policy/facade/native adapter and descriptor/deadline management remain independently implemented. OpenSSL setup/error source was inspected to diagnose allocation failures; no implementation or comments were copied. |
...
| N1 Milestone 6 wget application and literal-factory extension | Original project protocol helpers and independent address facade | `MIT OR Apache-2.0` | RFC 3986, RFC 9110 and RFC 9112 behavioral review in `STDLIB_N1_SOURCE_REVIEW.md`; no OpenJDK URI/HTTP source, comments or tests copied. Existing separately derived IP/SOCKS helpers are unchanged. Pinned OpenSSL record source was inspected to attribute existing native allocations; no source or comments were copied. |
...
The fixed en_US casing helper, generated tables, Instant calendar helper and
private IP literal parser and SOCKS wire helper below are OpenJDK-derived.
...
| --- | --- | --- | --- | --- | --- |
| `stdlib/src/main/ironwood/ironwood/net/SocksProtocol.iron` | `src/java.base/share/classes/java/net/SocksSocketImpl.java` | `060c4f7589e7f13febd402f4dac3320f4c032b08` (jdk21u, 21.0.1+12) | Complete Oracle header with Classpath Exception retained | 2026-09-16 | SOCKS4/5 wire negotiation only; NP9/NP10 explicit version/authentication, exact credential bytes, bounded reusable scratch, monotonic shared deadline, strict reply validation; no selector, ambient credentials, protocol fallback, descriptor lifecycle or reflection |
| `stdlib/src/main/ironwood/ironwood/net/IpLiteralParser.iron` | `src/java.base/share/classes/sun/net/util/IPAddressUtil.java` | `060c4f7589e7f13febd402f4dac3320f4c032b08` (jdk21u, 21.0.1+12) | Complete Oracle header with Classpath Exception retained | 2026-09-15 | Decimal IPv4, BSD ambiguity and IPv6 literal parsing only; fixed NP3, borrowed String ranges, primitive IPv4 result and caller-provided IPv6 bytes; no parser allocation, URL helpers, properties or interface cache |
...
| --- | --- | --- | --- | --- |
| Prepared SDK `sources/openssl-3.5.8.tar.gz`, `lib/libssl.a`, `lib/libcrypto.a`, `include/openssl/` | [OpenSSL release source](https://github.com/openssl/openssl/releases/download/openssl-3.5.8/openssl-3.5.8.tar.gz) | 3.5.8; SHA-256 `a8f84a39918ec6415ce765d9b429d313ba97b8143169c172e734b9514464f5b2` | `Apache-2.0` | Unmodified source built by `scripts/prepare-tls.py`; full source and upstream license ship in the SDK. Application dependency separate from Conda's OpenSSL. |
| Prepared SDK `share/cacert.pem`, generated `share/ironwood_ca_data.h` | [curl Mozilla CA export](https://curl.se/ca/cacert-2026-08-13.pem) | 2026-08-13; SHA-256 `f66dff1bdf8f96060b8177976f8b7d9254bc89bc4db933d769f7384d28480bc9` | `MPL-2.0` | Original export with notices retained. Recipe embeds the certificate PEM blocks as a C string; export, generated form, recipe and MPL text ship together. |
